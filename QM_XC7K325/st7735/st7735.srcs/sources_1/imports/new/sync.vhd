...
architecture rtl of sync_t1 is
  constant DIS_H_PXL    : natural := 160;  -- num of H pixels per display
  constant DIS_V_PXL    : natural :=  80;  -- num of V pixels per display
  constant CHR_H_PXL    : natural :=   8;  -- num of H pixels per char
  constant CHR_V_PXL    : natural :=  12;  -- num of V pixels per char
  constant PXL_H_BIT    : natural :=  16;  -- num of H bits   per pixel
  constant BIT_H_CLK    : natural :=   2;  -- num of H clks   per bit
  constant DIS_H_CHR    : natural := DIS_H_PXL/ CHR_H_PXL ;                           -- num of H chars per display 
  constant CHR_H_CLK    : natural := CHR_H_PXL * PXL_H_BIT * BIT_H_CLK;               -- num of H clks per char
  constant DIS_H_CLK    : natural := DIS_H_CHR * CHR_H_PXL * PXL_H_BIT * BIT_H_CLK;   -- num of H clks per display
  constant DIS_V_BIT    : natural := DIS_V_PXL;                                       -- num of V bits per display
...
-- return : char vertical row
function get_chr_v ( vctr : in integer) return integer is
begin
      if    ( vctr >= 6*CHR_V_PXL - 1 ) then return 6;
      elsif ( vctr >= 5*CHR_V_PXL - 1 ) then return 5; 
      elsif ( vctr >= 4*CHR_V_PXL - 1 ) then return 4; 
      elsif ( vctr >= 3*CHR_V_PXL - 1 ) then return 3; 
      elsif ( vctr >= 2*CHR_V_PXL - 1 ) then return 2; 
      elsif ( vctr >=   CHR_V_PXL - 1 ) then return 1; 
      else                                   return 0; 
      end if;
end function; 

-- Function get_chr_v_row
-- input  : vertical counter
-- return : pixel vertical row
function get_chr_v_row ( vctr : in integer) return integer is
begin 
      if    ( vctr < CHR_V_PXL) then return vctr;
      elsif ( vctr < CHR_V_PXL) then return vctr -   CHR_V_PXL; 
      elsif ( vctr < CHR_V_PXL) then return vctr - 2*CHR_V_PXL; 
      elsif ( vctr < CHR_V_PXL) then return vctr - 3*CHR_V_PXL; 
      elsif ( vctr < CHR_V_PXL) then return vctr - 4*CHR_V_PXL; 
      elsif ( vctr < CHR_V_PXL) then return vctr - 5*CHR_V_PXL; 
      else                           return vctr - 6*CHR_V_PXL; 
      end if;
 end function;

-- Function get_chr_h
-- input  : hctr = horizontal counter
-- return : char horiontal column
  ...
  begin
    if reset = '1' then
      ...
      if( delay > 0) then
        ...
        elsif (delay = 1 )   then sel_reg <= "000" ; clkr <= '1';
        else
        end if;
        if(ready = '1') then 
          delay <= delay - 1;
